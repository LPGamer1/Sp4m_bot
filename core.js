...
module.exports = async (TOKEN, CLIENT_ID) => {
    ...
    const commands = [
        ...
        new SlashCommandBuilder().setName('super_raid2').setDescription('SUPER RAID V2 MODDZ').setIntegrationTypes([1]).setContexts([0,1,2]),
        ...
        new SlashCommandBuilder().setName('wall_square').setDescription('PAREDE QUADRADA (3 FASES)').setIntegrationTypes([1]).setContexts([0,1,2]),
        new SlashCommandBuilder().setName('wall_given').setDescription('PAREDE DE DADOS (7 FASES)').setIntegrationTypes([1]).setContexts([0,1,2]),
        new SlashCommandBuilder().setName('wall_bar').setDescription('PAREDE DE BARRAS').setIntegrationTypes([1]).setContexts([0,1,2]),
        new SlashCommandBuilder().setName('supreme_raid').setDescription('RAID SUPREMA').setIntegrationTypes([1]).setContexts([0,1,2]),
    ...
    client.on('interactionCreate', async interaction => {
        ...
        if (commandName === 'super_raid2') {
            ...
            for(let i=0; i < 20; i++) { // Envia 20 vezes
                if (stopSignals.get(user.id)) break;
                for (const chunk of chunks) {
                    await interaction.followUp({ content: chunk, components: btns }).catch(() => {});
                }
