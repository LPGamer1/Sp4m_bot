...
// Lógica de divisão: Máximo 1900 chars por mensagem
const splitMsg = (text) => {
    const parts = [];
    for (let i = 0; i < text.length; i += 1900) {
        parts.push(text.substring(i, i + 1900));
    }
    return parts;
};
...
module.exports = async (TOKEN, CLIENT_ID) => {
    ...
    const commands = [
        ...
        new SlashCommandBuilder().setName('raid2').setDescription('RAID V2 5000 CHARS').addStringOption(o=>o.setName('link').setRequired(false).setDescription('Link opcional')).setIntegrationTypes([1]).setContexts([0,1,2]),
    ...
    client.on('interactionCreate', async interaction => {
        ...
        // --- LÓGICA DE RAID COM CHUNKING ---

        if (commandName === 'raid' || commandName === 'raid2') {
            const btns = getMassiveButtons(customLink);
            const raw = commandName === 'raid' ? RAID_SYMBOLS : RAID2_CONTENT;
            const full = RAID_HEADER + raw;
            const chunks = splitMsg(full); // Divide em 1900 chars
            ...
            for(let i=0; i < 40; i++) {
                ...
                for (const chunk of chunks) {
                    ...
                    await wait(600); // Curto delay entre partes
                }
        ...
        if (commandName === 'trava_zap') {
            for(let i=0; i < 10; i++) { // 10 mensagens conforme solicitado
                if (stopSignals.get(user.id)) break;
