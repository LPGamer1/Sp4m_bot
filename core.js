...
// Símbolos limpos (sem emojis) + Símbolos esquisitos no final
...
module.exports = async (TOKEN, CLIENT_ID) => {
    ...
    const commands = [
        new SlashCommandBuilder().setName('raid').setDescription('RAID EXTREMA 2000 CHARS')
        ...
        new SlashCommandBuilder().setName('god').setDescription('RAID RELIGIOSA')
            .addStringOption(o=>o.setName('link').setRequired(false).setDescription('Link opcional'))
            .setIntegrationTypes([1]).setContexts([0,1,2]),
        new SlashCommandBuilder().setName('stop').setDescription('Para o bot')
            .setIntegrationTypes([1]).setContexts([0,1,2])
    ].map(c => c.toJSON());
    ...
    client.on('interactionCreate', async interaction => {
        ...
        if (commandName === 'stop') {
            ...
            return interaction.reply({ content: '🛑 **TERMINADO.**', ephemeral: true });
        }

        await interaction.reply({ content: '💀 **Executando...**', ephemeral: true }).catch(() => {});
        ...
        if (commandName === 'raid') {
            ...
            // Corta para garantir o limite de 2000 chars (Header + Símbolos)
            const raidMsg = (HEADER + RAID_SYMBOLS).substring(0, 1990); 
